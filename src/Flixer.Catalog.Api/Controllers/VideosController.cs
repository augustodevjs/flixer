...
public class VideosController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateVideo(
        [FromBody] CreateVideoInput request,
        CancellationToken cancellationToken)
    {
        var output = await _mediator.Send(request, cancellationToken);
        
        return CreatedAtAction(nameof(CreateVideo), new { id = output.Id }, new ApiResponse<VideoOutput>(output));
    }
    ...
    public async Task<IActionResult> UpdateVideo(
        [FromRoute] Guid id,
        [FromBody] UpdateVideoInput input,
        CancellationToken cancellationToken)
    {
        var result = await _mediator.Send(input, cancellationToken);
        var output = new ApiResponse<VideoOutput>(result);
        return Ok(output);
    }
